...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        for batch_idx, interaction in enumerate(train_data):
            ...
            with torch.no_grad():
                if self.config['model'] == 'SWAV_Diff':
                    w = self.model.prototypes.weight.data.clone()
                    w = torch.nn.functional.normalize(w, dim=1, p=2)
                    self.model.prototypes.weight.copy_(w)
                if self.config['model'] == 'SWAV_Diff_Clusters:':
                    w_u = self.model.prototype_u.weight.data.clone()
                    w_u = torch.nn.functional.normalize(w_u, dim=1, p=2)
                    self.model.prototype_u.weight.copy_(w_u)

                    w_i = self.model.prototype_i.weight.data.clone()
                    w_i = torch.nn.functional.normalize(w_i, dim=1, p=2)
                    self.model.prototype_u.weight.copy_(w_i)
            ...
            # cancel gradients for the protoytpes
            # iteration = epoch_idx * len(train_data) + batch_idx
            if 'SWAV' in self.config['model']:
                if epoch_idx < 1:
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if verbose:
                ...
                if post_info is not None:
                    self.logger.info(self.model.post_epoch_processing())
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, is_test=False, idx=0):
        ...
        for batch_idx, batched_data in enumerate(eval_data):
            ...
            masked_items = batched_data[1] # mask train items
            # mask out pos items
